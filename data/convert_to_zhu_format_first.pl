...
while ($biased ne 0) {
    ...
    `cat $trainFile.shuffle | head -n$trainsize > $trainFile.$trainsize.shuffle`;
    $biased = `perl is_biased_file.pl $trainFile.$trainsize.shuffle`;
}
`cat $trainFile.shuffle | tail -n+$trainsize > $trainFile.$trainsize.comp.shuffle`;
`rm $trainFile.shuffle`;
